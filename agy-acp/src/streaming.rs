...
use crate::protobuf::{
    extract_text_from_step_payload, extract_tool_from_step_payload, is_tool_step_type,
    tool_call_title,
};
...
/// Replay existing conversation history for a loaded session.
pub fn replay_history(
    ...
    let mut stmt = match conn.prepare(
        "SELECT idx, step_type, step_payload FROM steps WHERE step_type = 15 OR step_type IN (5,7,8,9,17,21,33,101,138) ORDER BY idx"
    ) {
        Ok(s) => s,
        Err(_) => return Vec::new(),
    };
